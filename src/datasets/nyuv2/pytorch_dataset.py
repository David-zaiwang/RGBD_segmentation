...
class NYUv2(NYUv2Base, DatasetBase):
    ...
    def load_image(self, idx):
        img_path = os.path.join(self._data_dir, self._filenames[idx][0])

        return self._load_114(img_path)

    def load_depth(self, idx):
        depth_path = os.path.join(self._data_dir, self._filenames[idx][1]).replace('Label', 'ColoredLabel')
        assert os.path.exists(depth_path), depth_path
        im = cv2.imread(depth_path, 0)
        return im

    def load_label(self, idx):
        label_path = os.path.join(self._data_dir, self._filenames[idx][1])
        im = cv2.imread(label_path, 0)
        return im
